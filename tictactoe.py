...
def est_signe_seul(saisie):
    for i in range(len(saisie)):
        if not est_lettre(saisie[i]) and not est_chiffre(saisie[i]):
            return False
    return True
...
def test_entier(saisie):
    return est_signe_seul(saisie) and est_lettre(saisie) and est_signe_multiple(saisie) and est_signe_espace(saisie) and est_chiffre_signe(saisie) and chiffre_espace_chiffre(saisie)
